 ...
 * end up as rows in wp_jsl_enrollments, and Guide\Access\Access is the only
...
class Subscription {
	...
	public static function handle_subscribe( \WP_REST_Request $request ) {
		if ( ! self::is_enabled() ) {
			return new \WP_REST_Response( array( 'error' => __( 'Subscriptions are not available.', 'guide-lms' ) ), 404 );
		}
	...
	public static function create_session( int $user_id ) {
		...
		if ( $code >= 400 || empty( $data['checkout_url'] ) ) {
			...
			return new \WP_Error( 'jsl_dodo_error', __( 'Could not start checkout. Please try again.', 'guide-lms' ) );
		}

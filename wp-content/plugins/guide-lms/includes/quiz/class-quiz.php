 ...
 *
 * Routes:
 *  GET  guide/v1/lessons/{id}/quiz        public questions (options only, no answers)
 *  POST guide/v1/lessons/{id}/quiz/grade  logged-in; grades server-side, marks the
 *                                       lesson complete on a passing score
 *  GET  guide/v1/lessons/{id}/quiz-admin  editors: full quiz for the builder
 *  POST guide/v1/lessons/{id}/quiz-admin  editors: save full quiz
 */

namespace Guide\Quiz;
...
class Quiz {
	...
	public static function get_public( \WP_REST_Request $request ) {
		...
		if ( ! \Guide\Access\Access::can_view_lesson( $lesson_id ) ) {
	...
	public static function grade( \WP_REST_Request $request ) {
		...
		if ( $passed && $course_id && class_exists( 'Guide\\Progress\\Progress' ) ) {
			\Guide\Progress\Progress::complete( get_current_user_id(), $lesson_id, $course_id );
			$progress = \Guide\Progress\Progress::course_progress( get_current_user_id(), $course_id );
		}

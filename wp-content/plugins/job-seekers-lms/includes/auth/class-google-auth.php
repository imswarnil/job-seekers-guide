...
class Google_Auth {
	...
	public static function route() {
		...
		if ( ! self::is_enabled() ) {
			self::fail( __( 'Google sign-in is not configured.', 'job-seekers-lms' ) );
		}
	...
	private static function handle_callback() {
		...
		if ( ! $state || ! $code || ! $cookie_state || ! hash_equals( $cookie_state, $state ) ) {
			self::fail( __( 'Sign-in could not be verified. Please try again.', 'job-seekers-lms' ) );
		}
		...
		if ( ! is_array( $stash ) || empty( $stash['verifier'] ) ) {
			self::fail( __( 'Your sign-in link expired. Please try again.', 'job-seekers-lms' ) );
		}
	...
	private static function exchange_code( string $code, string $verifier ) {
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error( 'jsl_google_network', __( 'Could not reach Google. Please try again.', 'job-seekers-lms' ) );
		}
		...
		if ( wp_remote_retrieve_response_code( $response ) >= 400 || empty( $body['id_token'] ) ) {
			...
			return new \WP_Error( 'jsl_google_token', __( 'Google sign-in failed. Please try again.', 'job-seekers-lms' ) );
		}
	...
	private static function validate_id_token( string $id_token ) {
		...
		if ( 3 !== count( $parts ) ) {
			return new \WP_Error( 'jsl_google_jwt', __( 'Google sign-in failed. Please try again.', 'job-seekers-lms' ) );
		}
		...
		if ( ! $issuer_ok || ! $audience_ok || ! $unexpired ) {
			return new \WP_Error( 'jsl_google_claims', __( 'Google sign-in could not be verified.', 'job-seekers-lms' ) );
		}

		if ( empty( $claims['sub'] ) || empty( $claims['email'] ) ) {
			return new \WP_Error( 'jsl_google_claims', __( 'Google did not share an email address.', 'job-seekers-lms' ) );
		}
		...
		if ( empty( $claims['email_verified'] ) || ! filter_var( $claims['email_verified'], FILTER_VALIDATE_BOOLEAN ) ) {
			return new \WP_Error( 'jsl_google_unverified', __( 'Please verify your email with Google first.', 'job-seekers-lms' ) );
		}
	...
	private static function resolve_user( array $claims ) {
		...
		if ( ! self::allows_signup() ) {
			return new \WP_Error( 'jsl_google_no_signup', __( 'This site is not accepting new Google sign-ups.', 'job-seekers-lms' ) );
		}
	...
	public static function render_login_button() {
		...
		$redirect_to = isset( $_GET['redirect_to'] ) ? esc_url_raw( wp_unslash( $_GET['redirect_to'] ) ) : '';
		?>
		<div class="jsl-google-signin">
			<div class="jsl-google-signin__divider"><span><?php esc_html_e( 'or', 'job-seekers-lms' ); ?></span></div>
			<a class="jsl-google-signin__btn" href="<?php echo esc_url( self::start_url( $redirect_to ) ); ?>">
				...
				<?php esc_html_e( 'Continue with Google', 'job-seekers-lms' ); ?>
			</a>
		</div>
		<?php
	}
	...
	private static function fail( string $message ) {
		wp_die(
			esc_html( $message ),
			esc_html__( 'Sign-in failed', 'job-seekers-lms' ),
			array(
				'response'  => 400,
				'back_link' => true,
			)

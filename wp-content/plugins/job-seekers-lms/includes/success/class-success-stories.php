...
class Success_Stories {
	...
	public static function register() {
		register_post_type(
			...
			array(
				'labels'             => array(
					'name'          => __( 'Success Stories', 'job-seekers-lms' ),
					'singular_name' => __( 'Success Story', 'job-seekers-lms' ),
				),
				'public'             => true,
	...
	public static function submit( \WP_REST_Request $request ) {
		if ( ! self::is_enabled() ) {
			return new \WP_REST_Response( array( 'error' => __( 'Story submissions are closed.', 'job-seekers-lms' ) ), 403 );
		}
		...
		if ( ! empty( $pending ) ) {
			return new \WP_REST_Response(
				array( 'error' => __( 'You already have a story waiting to be reviewed.', 'job-seekers-lms' ) ),
				429
			);
		}
	...
	public static function list_for_console( \WP_REST_Request $request ) {
		...
		foreach ( $stories as $story ) {
			...
			$out[] = array(
				...
				'author'    => $author ? $author->display_name : __( 'Unknown', 'job-seekers-lms' ),

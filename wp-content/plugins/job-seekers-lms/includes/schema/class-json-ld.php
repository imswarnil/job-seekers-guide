...
class Json_Ld {
	...
	private static function course( int $course_id ): array {
		...
		$stats   = \JSL\Course_Api::get_stats( $course_id );

		$lessons  = array();
		$position = 0;
		foreach ( \JSL\Course_Api::get_lessons_flat( $course_id ) as $lesson ) {
			$position++;
	...
	private static function lesson( int $lesson_id ): array {
		...
		$embed     = $video_url && class_exists( 'JSL\\Lesson_Meta' ) ? \JSL\Lesson_Meta::embed_info( $video_url ) : null;
	...
	private static function learning_path( int $path_id ): array {
		$items    = array();
		$position = 0;
		foreach ( \JSL\Course_Api::get_path_courses( $path_id ) as $course ) {
			$position++;
			$items[] = array(
				'@type'    => 'ListItem',
				'position' => $position,

...
class Pwa {
	...
	private static function serve_manifest() {
		...
		$manifest = array(
			...
			'shortcuts'        => array(
				array(
					'name' => __( 'My Learning', 'job-seekers-lms' ),
					'url'  => home_url( '/my-learning/' ),
				),
				array(
					'name' => __( 'Browse courses', 'job-seekers-lms' ),
	...
	private static function serve_service_worker() {
		...
		$config = wp_json_encode(
			array(
				'version'     => self::SW_VERSION . '.' . JSL_VERSION,
		...
		readfile( JSL_PLUGIN_DIR . 'includes/pwa/service-worker.js' );
		exit;
	}
...
<head>
	...
	<title><?php esc_html_e( 'You’re offline', 'job-seekers-lms' ); ?> — <?php bloginfo( 'name' ); ?></title>
	<style>
...
<body>
	<main>
		...
		<h1><?php esc_html_e( 'You’re offline', 'job-seekers-lms' ); ?></h1>
		<p><?php esc_html_e( 'Lessons you have already opened are still available. Reconnect to load anything new.', 'job-seekers-lms' ); ?></p>
		<a href="<?php echo esc_url( home_url( '/my-learning/' ) ); ?>"><?php esc_html_e( 'Go to My Learning', 'job-seekers-lms' ); ?></a>
	</main>
</body>
</html>
		<?php
	...
	public static function enqueue_registration() {
		...
		wp_register_script( 'jsl-pwa', '', array(), JSL_VERSION, true );
		wp_enqueue_script( 'jsl-pwa' );

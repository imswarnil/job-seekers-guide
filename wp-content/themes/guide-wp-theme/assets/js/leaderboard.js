...
( function () {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		var toggle = document.getElementById( 'guide-lb-optin' );

		if ( ! toggle || ! window.guideLeaderboard ) {
			return;
		}
		...
		toggle.addEventListener( 'change', function () {
			...
			fetch( window.guideLeaderboard.restUrl + '/leaderboard/opt-out', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': window.guideLeaderboard.nonce },
				...
				.then( function ( res ) {
					...
					if ( window.guideSnackbar ) {
						window.guideSnackbar( window.guideLeaderboard.i18n[ shown ? 'shown' : 'hidden' ] );
					}
				} )
				.catch( function () {

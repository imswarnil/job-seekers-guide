...
add_action( 'wp_head', 'guide_theme_dark_mode_bootstrap', 0 );

function guide_theme_dark_mode_bootstrap() {
	?>
	<script>
	(function () {
		try {
			var stored = localStorage.getItem('guide-theme');
			...
			window.guideSetThemeMode = function (next) {
				mode = next;
